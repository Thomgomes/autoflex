...
describe('Product Material Slice', () => {
  ...
  it('should return the initial state', () => {
    expect(productMaterialReducer(undefined, { type: 'unknown' })).toEqual(initialState);
  });

  // Testando o Reducer Síncrono (clearRecipe)
  it('should clear recipe items using clearRecipe action', () => {
    const stateWithItems = {
      ...initialState,
      recipeItems: [{ id: 1, productId: 1, materialId: 10, quantityRequired: 5 }]
    };
  ...
  // Testando busca de receita (Pending)
  ...
  // Testando busca de receita (Fulfilled)
  it('should handle fetchRecipeByProduct.fulfilled', () => {
    const mockRecipe = [
      { id: 1, productId: 1, materialId: 10, quantityRequired: 5 }
    ];
    const action = { type: fetchRecipeByProduct.fulfilled.type, payload: mockRecipe };
    const state = productMaterialReducer(initialState, action);
  ...
  // Testando adicionar item à receita
  it('should add an item to the recipe when addMaterialToRecipe is fulfilled', () => {
    const newItem = { id: 2, productId: 1, materialId: 20, quantityRequired: 2 };
    const action = { type: addMaterialToRecipe.fulfilled.type, payload: newItem };
    const state = productMaterialReducer(initialState, action);
  ...
  it('should remove a material from the recipe when removeMaterialFromRecipe is fulfilled', () => {
    // 1. Criamos um estado inicial que já contém itens na receita
    const stateWithItems = {
      ...initialState,
      recipeItems: [
        { id: 1, productId: 1, materialId: 10, quantityRequired: 5 },
    ...
    // 2. Criamos a ação de sucesso da remoção (passando o ID do item a ser removido)
    const action = { 
    ...
    // 4. Verificamos se o item com ID 1 foi removido e se o ID 2 permanece
    expect(state.recipeItems).toHaveLength(1);
